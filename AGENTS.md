...
- Keep test-plan JSON command payloads actionable for each scenario (not just repository prechecks), or Codex test executors may fail for lack of executable evidence collection steps.
- Maintain compatibility aliases in QA artifacts/executor for field naming drift (`passFail`/`passCriteria`, `retryRules`/`retryRule`) to avoid deterministic schema-gate regressions.
- Keep objective-mode evidence complete for TC-004: maintain at least one `test-plan-objective-<type>-*.md` artifact for each allowed type (`acceptance`, `bug`, `test-gap`, `flaky`, `refactor-for-testability`, `performance`) plus deterministic rejection artifacts for unsupported and ambiguous objectives with clear guidance.
- For TC-015 terminal-failure coverage, use a runtime command that executes `qa-codex-loop.sh` against a deterministic failing fixture and validates `summary.json` (`status=FAIL`, loop-limit `stopReason`, non-empty `unresolved`, and `lastRootCauses` field presence); static `rg`-only checks are insufficient.
